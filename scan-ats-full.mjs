...
// ── Liveness verification (uses liveness-browser.mjs) ───────────────────────────

async function filterLive(offers) {
  let acquireBrowser, releaseBrowser, checkUrlLiveness, newLivenessPage;
  try {
    // browser.mjs imports Playwright statically, so this throws (caught below)
    // with a helpful message when Chromium isn't installed.
  ...
  // CDP_URL connects to an already-running browser; otherwise launch headless.
...
async function main() {
  ...
  for (const name of opts.ats) {
    ...
    console.log(`\n⚙️  ${name} — ${entries.length} companies`);
